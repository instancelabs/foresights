...
import type { Deps, GitHubSource, Source, Spotlight } from '../types';
import { createInMemoryStorage } from '../util/storage';
import { buildRefreshPrompt, refreshSpotlightsViaHaiku } from './refresh';

// Typed as readonly GitHubSource[] (the narrower variant of Source) so spread
// fixtures below can be re-asserted as GitHubSource without `kind === 'rss'`
// branch noise. Add an explicit RssSource fixture if/when refresh starts
// sampling RSS feeds.
const sampleSources: readonly GitHubSource[] = [
  {
    id: 'cdk-core',
    label: 'aws/aws-cdk',
    owner: 'aws',
