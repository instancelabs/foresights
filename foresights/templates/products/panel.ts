...
/**
 * Pure HTML for the registry-driven action block (summary / task). The
 * claude-code action keeps its own verbatim block inside `renderBriefHtml`.
 */
const renderActionBlock = (spec: ActionTypeSpec, briefId: string): string => {
  const id = escHtml(briefId);
  const copyBtns = spec.copyFormats
    .map((f) =>
      f === 'task'
        ? '<button class="brief-cc-copy" type="button" data-format="task" title="Copy as a markdown task file">Copy as task</button>'
        : '<button class="brief-cc-copy" type="button" data-format="prompt" title="Copy the text">Copy</button>',
    )
    .join('');
  return `<div class="brief-cc-actions"><button class="brief-cc-btn" type="button" data-brief-id="${id}">${escHtml(spec.actionLabel)}</button></div><div class="brief-cc-panel hidden" data-brief-id="${id}" data-mode="plan"><div class="brief-cc-header"><strong>${escHtml(spec.panelTitle)}</strong><div class="brief-cc-controls">${copyBtns}</div></div><pre class="brief-cc-text"></pre></div>`;
};

/**
 * Pure HTML for the brief body — why / integrations / action-block scaffold /
 * footer. Exported for unit testing without DOM mount.
 *
 * `actionType` is the product's action type; absent / `'claude-code'` emits
 * the verbatim cc-prompt block (gated on `hasCcBuilder`), so a claude-code
 * dashboard's brief HTML is byte-identical to pre-Phase-10.5 builds.
 */
export const renderBriefHtml = (
  ...
  if (at === 'claude-code') {
    ccBlock = hasCcBuilder
      ? `<div class="brief-cc-actions"><button class="brief-cc-btn" type="button" data-brief-id="${escHtml(briefId)}">Generate coding agent prompt</button></div><div class="brief-cc-panel hidden" data-brief-id="${escHtml(briefId)}" data-mode="plan"><div class="brief-cc-header"><strong>Coding agent prompt</strong><div class="brief-cc-controls"><div class="cc-mode-toggle" role="group" aria-label="Prompt mode"><button type="button" class="active" data-mode="plan" title="Ask the coding agent to produce a plan and wait for approval">Plan</button><button type="button" data-mode="implement" title="Ask the coding agent to plan and implement">Plan + Implement</button></div><button class="brief-cc-copy" type="button" data-format="prompt" title="Copy the prompt text">Copy</button><button class="brief-cc-copy" type="button" data-format="task" title="Copy as a markdown task file">Copy as task</button></div></div><pre class="brief-cc-text"></pre></div>`
      : '';
  } else {
    ccBlock = renderActionBlock(ACTION_TYPES[at], briefId);
  }
  const footer = `<div class="footer"><span>Generated by Haiku · cached locally</span>${
    url ? `<a href="${safeHref(url)}" target="_blank" rel="noopener">Open source ↗</a>` : ''
  }</div>`;
...
export const initBriefPanel = (deps: Deps, opts: InitBriefPanelOpts): PanelHandle => {
   ...
   * prompt with the refreshed repo-context block appended (the verbatim
   * pre-Phase-10.5 path); summary / task → the registry builder. Returns
   * null when a claude-code product has no registered builder, so callers
   * keep the pre-Phase-10.5 no-op behaviour.
   */
  const buildActionText = (entry: BriefEntry, mode: 'plan' | 'implement'): string | null => {
    const at = actionTypeFor(entry.flag.productId);
    if (at === 'claude-code') {
      const builder = ccBuilders[entry.flag.productId];
      if (!builder) return null;
      return appendRepoContext(
        builder({ brief: entry.brief, meta: entry.flag, mode }),
        formatRepoContext(deps, opts.topicSlug, entry.flag.productId),
      );
    }
    return ACTION_TYPES[at].build({ brief: entry.brief, meta: entry.flag });
  };
  ...
  const expandBrief = async (span: HTMLElement, host: HTMLElement): Promise<void> => {
    ...
    try {
      ...
      panel.innerHTML = renderBriefHtml(
        brief,
        productLabel,
        urlStr,
        briefId,
        hasCcBuilder,
        actionTypeFor(flag.productId),
      );
    } catch (err) {
  ...
  const onCcBtnClick = (btn: HTMLElement): void => {
    ...
    if (!panel) return;
    const spec = ACTION_TYPES[actionTypeFor(entry.flag.productId)];
    if (panel.classList.contains('hidden')) {
      refreshPanelText(panel, briefId);
      panel.classList.remove('hidden');
      btn.textContent = spec.hideLabel;
    } else {
  ...
  const onCopyClick = async (btn: HTMLElement): Promise<void> => {
    ...
    // claude-code builds the per-product CC prompt (with the refreshed
    // repo-context block appended); summary / task build via the registry.
    const built = buildActionText(entry, mode);
  ...
  /** Re-render the action text inside `panel` from the registry entry. */
  const refreshPanelText = (panel: HTMLElement, briefId: string): void => {
    const entry = lookupBrief(briefId);
    if (!entry) return;
    const mode = readMode(dataStr(panel, 'mode'));
    const text = buildActionText(entry, mode) ?? '';
