...
export const initBriefPanel = (deps: Deps, opts: InitBriefPanelOpts): PanelHandle => {
  ...
  const onCopyClick = async (btn: HTMLElement): Promise<void> => {
    ...
    if (!builder) return;
    // Append the refreshed repo-context block (empty unless the user has
    // clicked ↻ for this product) before any task-wrapping, so the context
    // rides inside the task body too.
    const built = appendRepoContext(
