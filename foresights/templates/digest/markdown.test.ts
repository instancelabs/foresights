...
describe('renderDigestMarkdown — repoContext', () => {
  ...
  it('appends the repoContext block to the embedded cc-prompt when supplied', () => {
    const md = renderDigestMarkdown({
      ...BASE,
      entries: [entry('a')],
      triaged: [triaged('a', 'green')],
      ccBuilder,
      repoContext: '## Repo context (refreshed 2026-05-20)\n- `src/rules` — directory, 31 entries',
    });
    expect(md).toContain('BUILT_PROMPT');
    expect(md).toContain('## Repo context (refreshed 2026-05-20)');
    expect(md).toContain('`src/rules` — directory, 31 entries');
    // The block sits inside the same fenced code block as the prompt.
    expect(md).toMatch(/```\nBUILT_PROMPT\n\n## Repo context/);
  });

  it('leaves the cc-prompt unchanged when repoContext is absent', () => {
    const md = renderDigestMarkdown({
      ...BASE,
      entries: [entry('a')],
      triaged: [triaged('a', 'green')],
    ...
    expect(md).toMatch(/```\nBUILT_PROMPT\n```/);
    expect(md).not.toContain('## Repo context');
  });

  it('does not emit repoContext for red items (no embedded prompt)', () => {
    const md = renderDigestMarkdown({
      ...BASE,
      entries: [entry('a')],
      triaged: [triaged('a', 'red')],
