...
const renderDetailed = (
  ...
  rows.forEach(({ entry, triage }, idx) => {
    ...
    if (ccBuilder) {
      const prompt = ccBuilder({ brief: entry.brief, meta: entry.flag, mode: 'plan' });
      lines.push('');
      lines.push('<details>');
...
export const renderDigestMarkdown = (args: RenderDigestArgs): string => {
  const { productLabel, productSlug, date, entries, triaged, ccBuilder } = args;
  ...
  for (const bucket of ['green', 'yellow'] as const) {
    ...
    lines.push('');
    renderDetailed(lines, buckets[bucket], ccBuilder);
  }
